    ...
    <div id="peopleindex"  v-loading="lodingtable" element-loading-text="拼命加载中">
        <el-row :gutter="10">
            ...
            <el-col :span="20" style="padding: 4px;">
                ...
                <el-button size="small" v-if="authority.indexOf('staff_add')!==-1" @click="opendetail(false)" type="primary">新增</el-button>
        ...
        <el-table
            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    {{func.filterName(scope.row.fk_hpl_id,$store.getters.all_hpl_list)}}
                </template>
            </el-table-column>
...
export default {
    ...
    methods:{
        setBasicDate(){                         //设置基础数据
            api.getAllRolesList({type:1}).then(_=>{
        ...
        opendetail(row){                            //打开人员详情
            ...
            if(row){
                ...
                apii.getStaffInfo({pk_id:row.pk_id}).then(res=>{
                    ...
                    this.form.hplnames=func.filterName(res.data.staff_base_data.fk_hpl_id,this.$store.getters.all_hpl_list)
            ...
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    ...
                    if(this.form.pk_id){
                        ...
                        apii.setStaffAdd(data ).then(res=>{
                            this.detailDialogVisible=false
                            this.$message({
                                message: '新增成功',
                                type: 'success'
        ...
        licencePicSuccess(file,fileList){            //证件照片上传成功触发的事件
            setTimeout(()=>{
               ...
               // console.log(this.oss)
                this.formlicence.image_url=this.oss.host+'/'+this.oss.data.key
